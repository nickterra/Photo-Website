...
function onResourcesReady(response){
	...
	try
	{
		response.htmlRes["annotationBalloon.html"] = response.htmlRes["annotationBalloon.html"].replace(new RegExp("sacore:empty.gif","g"), response.imageDatamap["empty.gif"]);
		...
		if ( typeof(doCashback) != "undefined" )
			fnDoCashback = doCashback;
